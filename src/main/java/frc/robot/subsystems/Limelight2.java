...
public class Limelight2 extends SubsystemBase {
	...
	public Limelight2(XboxController joystick) {
		...
		xOffEntry = sb_limelight.add("X Offset", nt_xOffset.getDouble(0.0)).getEntry();
		yOffEntry = sb_limelight.add("Y Offset", nt_yOffset.getDouble(0.0)).getEntry();
	...
	public void periodic() {
		...
		angleToTarget = (kLimelight.mountAngle + nt_yOffset.getDouble(0.0)) * (Math.PI / 180);

		// Getting distance to target(s) using trigonometry
		lowTargetDist = (kLimelight.lowTargetHeight - kLimelight.heightOffFloor) / Math.tan(angleToTarget);
		highTargetDist = (kLimelight.highTargetHeight - kLimelight.heightOffFloor) / Math.tan(angleToTarget);

		// Updating data on Shuffleboard
		xOffEntry.setDouble(nt_xOffset.getDouble(0.0));
		yOffEntry.setDouble(nt_yOffset.getDouble(0.0));
	...
	public double getXOffset() {return nt_xOffset.getDouble(0.0);}

	/** Gets the Y position/offset */
	public double getYOffset() {return nt_yOffset.getDouble(0.0);}
	...
	public double[] getCrop() {return nt_crop.getDoubleArray(new double[] {2, 2, 2, 2});}

	/** Gets the distance from the target */
	public double getDistanceToTarget(int level) {
		if (level == 1) {return lowTargetDist;}
		else if (level == 2) {return highTargetDist;}
		else {return 0;}
	}
